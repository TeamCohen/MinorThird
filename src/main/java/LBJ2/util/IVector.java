...
/**
  * This class implements an expandable array of <code>int</code>s that should
  * be faster than java's <code>Vector</code>.
  *
  * @author Nick Rizzolo
 **/
public class IVector implements Cloneable, java.io.Serializable
{
   ...
   **/
  public IVector() { this(defaultCapacity); }
  ...
  public IVector(int[] v) {
    if (v.length == 0) vector = new int[defaultCapacity];
    else {
      vector = v;
      size = vector.length;
    }
   ...
   **/
  public IVector(IVector v) { this(v.toArray()); }
  ...
  /**
    * Retrieves the value stored at the specified index of the vector, or 0 if
    ...
    * @throws ArrayIndexOutOfBoundsException  When <code>i</code> &lt; 0.
   **/
  public int get(int i) { return get(i, 0); }
  ...
  /**
    ...
    * @throws ArrayIndexOutOfBoundsException  When <code>i</code> &lt; 0.
   **/
  public int set(int i, int v) { return set(i, v, 0); }
  ...
  /**
    ...
    * @throws ArrayIndexOutOfBoundsException  When <code>i</code> &lt; 0.
   **/
  public void increment(int i) { increment(i, 0); }

  /**
    * Increments the integer at the given index by 1.  If the given index is
    * greater than the vector's current size, the vector will expand to
    * accomodate it.
    *
    * @param i  The index of the value to increment.
    * @param d  The default value for other new indexes that might get
  ...
  /**
    ...
    * @throws ArrayIndexOutOfBoundsException  When <code>i</code> &lt; 0.
   **/
  public void decrement(int i) { decrement(i, 0); }

  /**
    * Decrements the integer at the given index by 1.  If the given index is
    * greater than the vector's current size, the vector will expand to
    * accomodate it.
    *
    * @param i  The index of the value to decrement.
  ...
  public int remove(int i) {
    ...
    if (i >= size)
      throw
        new ArrayIndexOutOfBoundsException(
            "LBJ: IVector: Can't remove element at index " + i
  ...
  public int max() {
    ...
    for (int i = 0; i < size; ++i) if (vector[i] > result) result = vector[i];
    return result;
  }
  ...
  /**
    * Searches this vector for the specified value using the binary search
    * algorithm.  This vector <strong>must</strong> be sorted (as by the
    * {@link #sort()} method) prior to making this call.  If it is not sorted,
    * the results are undefined.  If this vector contains multiple elements
    ...
    *         the vector if all elements in the list are less than
  ...
  /**
    * Returns a new array of <code>int</code>s containing the same data as
    * this vector.
   **/
  public int[] toArray() {
  ...
  /**
    * Two <code>IVector</code>s are considered equal if they contain the same
  ...
  public void write(ExceptionlessOutputStream out) {
    out.writeInt(size);
    for (int i = 0; i < size; ++i) out.writeInt(vector[i]);
  }
  ...
  public void read(ExceptionlessInputStream in) {
    size = in.readInt();
    if (size == 0) vector = new int[defaultCapacity];
    else {
      vector = new int[size];
      for (int i = 0; i < size; ++i) vector[i] = in.readInt();
    }
  }
}
