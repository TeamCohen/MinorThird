...
public class LabeledDirectory implements CommandLineProcessor.Configurable
{
    private static final FileFilter TEXT_FILE_FILTER = new FileFilter() {
	    public boolean accept(File file) { return file.getName().endsWith(".txt"); }
	};
    private File dir; 
    ...
    {
	public void dir(String s) { dir = getDirectory(s); }
	public void annotate(String s) { annotatorList.add(getAnnotator(s)); }
	public void require(String s) { requireList.add(s); }
	public void reset() { resetAll = true; }
	public void view(String s) { nameOfFileToView = s; }
    }
    ...
    public LabeledDirectory(String s) {dir=getDirectory(s);}
    public LabeledDirectory(File dir) {this.dir=dir;}
	...
	if (labelFile.exists()) return new TextLabelsLoader().loadOps( textBase, labelFile );
	else return new BasicTextLabels( textBase );
    }
	...
	if (textFiles==null) throw new IllegalArgumentException("can't list directory "+dir);
	...
	for (int j=0; j<textFiles.length; j++) {
	    ...
	    if (resetAll) labels = new BasicTextLabels(labels.getTextBase());
	    for (Iterator k=annotatorList.iterator(); k.hasNext(); ) {
		Annotator ann = (Annotator)k.next();
		ann.annotate(labels);
	    }
	    for (Iterator k=requireList.iterator(); k.hasNext(); ) {
		String req = (String)k.next();
