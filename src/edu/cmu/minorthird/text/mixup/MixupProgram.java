...
A Mixup program will look something like this:
 ...
 require "req1"; //requires that "abc" type spans have already been labeled.  If not, the default annoator<br>
                //for "abc" will be used.<br>
 require "req2", "req2.mixup"; //file 'def.mixup' will be run to provide "def" labels if they are not already there<br>
                             //if  "def" labels were already generated by a different annotator, they will be used and<br>
                             //and 'def.mixup' won't be called.<br>
 provide "xyz"; //this program will annotate the text with "xyz" labels<br>
 <br>
 defDict titleWord = mr, ms, mrs, dr; //defines a dictionary (with scope of this program execution called 'titleWord'<br>
                                      //containing the values "mr", "ms", "mrs", "dr" <br>
 defDict myDictionary = "dictionary.txt"; //defines a dictionary called 'myDictionary' with values taken from the file "dictionary.txt"<br>
 <br>
 defTokenProp title:true =: ... [ai(titleWord)] ... ; //finds all spans matching a work in the dictionary titleWord<br>
          //those spans are given the property "Name" with value "true" (a string, not boolean)<br>
          //if the span previously had "Name" property with a different value, that is replaced<br>
          // the "..." before and after indicate that it doesn't matter what comes before or after the token<br>
          //to be labeled.  if I said "=: [ai(titleWord)];" the document would need to be JUST a titleword.<br>
 <br>
 defTokenProp titlePunc:1 =: ... title:true [','] ... || ... title:true ['.'] ... ;<br>
          //spans "." or "," preceeded by a title are given the property titlePunc with value "1"<br>
          //note that the entire '... title:true [','] ...' is an expression; or operators ("||") must be<br>
          //<em> between</em> expressions, not within them<br>
  <br>
  defSpanType fullTitle =: ...[title:true titlePunc:1?R] ...;<br>
          //label a span as "fullTitle" if there is a title span optionally followed b a titlePunc span<br>
          //but not more than one (from the R)<br>
  <br>
  defSpanType the =: ... [eqi('the')] ...; //labels occurances of "the" ignoring case (eq = equals, adding i ignores case)<br>
  <br>
  defTokenProp aProp:t =: ...[<title:true, req1>] ...; //tokens which have the title=true property AND are labeled as req1<br>
          //are given the property aProp=t<br>
  defTokenProp address:x =: ... [@fullTitle any] !a(myDictionary) ...; //label spans of one 'fullTitle' (the @ is needed
          //before types) and the following token, whatever it is, which are followed by something other than a myDictionary word<br>
  defTokenProp capProp:on =req2: ... [re('^[A-Z]$')] ...; //on spans of type req2, match tokens fitting the given regular expression<br>
  <br>
  defSpanType listSet =: ... [address+R] ...; //label as header spans of 1 or more address tokens, going all the way to <br>
          //right most possible token - example: blah address1 address2 address3 blah - will return three spans: "address3",<br>
          //"address2 address3", and "address1 address2 address3"<br>
  defSpanType adList =: ... [L address+ R] ...; //as above but only returns the longest span<br>
  defSpanType header =: [L address* R] ...; //label longest span of 0 or more address tokens at the beginning of the document<br>
  defSpanType shortList =: ... [address{2,3}] ...; //label spans of 2 or 3 address tokens<br>
