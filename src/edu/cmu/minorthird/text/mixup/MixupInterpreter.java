...
public class MixupInterpreter {
    ...
    private HashMap levelsToLabelsMap = new HashMap();
    private Stack levelStack = new Stack();
    private TextBaseManager tbManager;    

    // Constructors
    public MixupInterpreter() {;}
    public MixupInterpreter(MixupProgram p) { program = p; }
    ...
    public void setProgram(MixupProgram p) { program = p; }

    /** Returns the MixupProgram that this interpreter will execute if the eval method is called. */
    public MixupProgram getProgram() { return program; }
    
    /** Returns the TextLabels associated with the given level name or null if the level doesn't exist */
    public MonotonicTextLabels getLabelsForLevel(String level) { return (MonotonicTextLabels)levelsToLabelsMap.get(level); }
    ...
    public MonotonicTextLabels getCurrentLabels() { return getLabelsForLevel(getCurrentLevel()); }
    ...
    public void createLevel(String newLevelName, String levelType, String pattern)  {
        ...
        MonotonicTextLabels parentLabels = (MonotonicTextLabels)levelsToLabelsMap.get(currentLevel);
	...
	if("pseudotoken".equals(levelType)) {
            ...
            Span.Looper typeInstances = parentLabels.instanceIterator(pattern);
            while (typeInstances.hasNext()) {
                Span currInstance = typeInstances.nextSpan();
                Span matchingChildSpan = tbManager.getMatchingSpan(currInstance, currentLevel, newLevelName);
        ...
        // creates a textBase which filters out all spans not of a certain spanType
            ...
            newTextBase = tbManager.filter(currentLevel, parentLabels, newLevelName, pattern);
            newLabels = new BasicTextLabels(newTextBase);
        }
	    ...
	    else tokenizer = new RegexTokenizer(pattern); //only things matching pattern will be tokens.
            newTextBase = tbManager.retokenize(tokenizer, currentLevel, newLevelName);
	    newLabels = new BasicTextLabels(newTextBase);
	} 
        else {
	...
	MonotonicTextLabels oldLabels = (MonotonicTextLabels)levelsToLabelsMap.get(importLevel);
        MonotonicTextLabels currLabels = this.getCurrentLabels();
	Span.Looper instances = oldLabels.instanceIterator(oldType);
	while(instances.hasNext()) {
	    Span currInstance = instances.nextSpan();
            Span newSpan = tbManager.getMatchingSpan(currInstance, importLevel, this.getCurrentLevel());
        ...
        if ("defDict".equals(keyword)) {
            ...
            if (statementType==Statement.MIXUP) {
                ...
                for (Iterator i=accum.iterator(); i.hasNext(); ) {
                    extendLabels( labels, ((Span)i.next()), statement);
                }
                ...
                while (input.hasNext()) {
                    ...
                    while (output.hasNext()) {
                        extendLabels( labels, output.nextSpan(), statement);
                    }
                }
