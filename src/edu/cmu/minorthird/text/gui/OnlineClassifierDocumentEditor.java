...
public class OnlineClassifierDocumentEditor extends ViewerTracker
{
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		else statusMsg.display("Document label cannot be changed - document already added");
	    } else statusMsg.display("Document has NOT been labeled");	      
	}
    }
	...
	if(nextCursor > -1) {
		...
		statusMsg.display("Cannot change label: Document has already been added to Classifier as: " + label);
		labelBox.setSelectedItem(label);
	    } else if(label != null) {
		statusMsg.display("Document is currently labeled: " + label);
		labelBox.setSelectedItem(label);
	    } else {
		statusMsg.display("In MOVEDOCCURSOI: This document has NOT been labeled");
