...
public class TextBaseLoader
{
    ...
    public TextBaseLoader(int documentStyle) { this.documentStyle = documentStyle; }
    ...
    // WARNING: All these constructors have been deprecated for a while now.  When should we remove?
    /** @deprecated */
    ...
    public MutableTextBase loadWordPerLineFile(File file) throws IOException, FileNotFoundException {
        ...
        ArrayList tokenPropList = new ArrayList();
        ...
        while((line = in.readLine()) != null) {
            ...
            // Otherwise we're at the end of a doc, so add it to the TextBase and continue
            else {
     ...
     */
    public void setLabelsInFile(boolean b) { this.use_markup = b; }
     ...
     */
    public void setDocumentStyle(int style) { this.documentStyle = style; }

    /** Sets whether the loader should recurse directories when loading docs. */
    public void setRecurseDirectories(boolean rec) { this.recurseDirectories = rec; }

    /** get labeling generated by tags in data file */
    public MutableTextLabels getLabels() { return labels; }


    //---------------Old Methods kept to support old api-------------------------------
    //WARNING: These are all deprecated.  How long have they been this way, should we delete them?
    /**
    ...
    private void loadFile(File file) throws IOException, ParseException {
        ...
        while (in.ready()) { //in.ready may cause problems on Macintosh
            ...
            // If this reader is set to create a doc for each line then add the doc now
     ...
     */
    private void addDocument(String docText, String documentId, ArrayList spans, ArrayList tokenProps) {
        //Blank documents are dropped
        if (docText.length() == 0) {
            log.warn("Text for document " + documentId +
        ...
        for (Iterator j=spans.iterator(); j.hasNext(); ) {
	    CharSpan charSpan = (CharSpan)j.next();
	    Span approxSpan; // = textBase.documentSpan(documentId).subSpan(charSpan.lo, charSpan.hi-charSpan.lo-1); 	   
	    boolean flag = false;
	    ...
	    if(flag)
                approxSpan = textBase.documentSpan(documentId).charIndexSubSpan(charSpan.lo, charSpan.hi);	      
	    else approxSpan = textBase.documentSpan(documentId).charIndexSubSpan(charSpan.lo, charSpan.hi).getLeftBoundary();
	    ...
	    Iterator itr = tokenProps.iterator();
	    if(tokens.length > 0) {		
     ...
     */
    protected String labelLine(String line, StringBuffer docBuffer, String docId, List spanList) throws ParseException {
        // stack of open tags
        if (stack == null)
            stack = new ArrayList();
        ...
        //BUG: THIS IS CAUSING BLANK LINES IN FILES TO BE ADDED AS DOCUMENTS WHEN LOADED in DOC_PER_LINE FORMAT
