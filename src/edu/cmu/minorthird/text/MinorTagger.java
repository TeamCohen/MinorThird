...
public class MinorTagger extends StringAnnotator {
  ...
  protected CharAnnotation[] annotateString(String text) {
    List charAnnList = new ArrayList();
...
//      if (text.trim().toLowerCase())
  ...
  private static CharAnnotation makeCharAnnotation(String text, String word, String pos, int startIndex, boolean caseSensitive) {
    ...
    else index = text.toLowerCase().indexOf(word.toLowerCase(), startIndex);
    if (index < 0)
      return null;
	...
	private static String tag (String in) {
		...
		// load text base
		TextBaseLoader baseLoader = new TextBaseLoader(TextBaseLoader.DOC_PER_FILE, true);
		File tempFile = createFile(tagged);
		TextBase base = null;
		try {
			base = baseLoader.load(tempFile);
		} catch (Exception e) {
