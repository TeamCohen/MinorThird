...
public class SpanDifference
{
	...
	public void predicted(String s) { predictedType=s; }
	public void actual(String s) { actualType=s; }
    }
	...
	if (inv.textLabels==null) throw new IllegalArgumentException("-labels must be set");
	if (inv.actualType==null) throw new IllegalArgumentException("-actual must be set");
	Span.Looper guess = inv.textLabels.instanceIterator(inv.predictedType);
	Span.Looper truth = inv.textLabels.instanceIterator(inv.actualType);
	Span.Looper closure = inv.textLabels.closureIterator(inv.actualType);
	if (guess==null) throw new IllegalArgumentException("spanType '"+inv.predictedType+"' not found");
	if (truth==null) throw new IllegalArgumentException("spanType '"+inv.actualType+"' not found");
