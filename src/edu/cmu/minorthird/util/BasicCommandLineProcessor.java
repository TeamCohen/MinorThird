...
abstract public class BasicCommandLineProcessor implements CommandLineProcessor,Saveable
{
    ...
    static private String[] loadOptionsInPropertiesFormat(File file) throws IOException
    {
	...
	while ((line = in.readLine())!=null) {
		...
		if (eqPos<0) inputError(in,line,file,"no equal-sign (=) in line");
		else {
		    String option = line.substring(0,eqPos).trim();
		    String value = line.substring(eqPos+1).trim();
		    accum.add("-"+option);
		    if (value.length()>0) accum.add(value);
		}
	    }
	}
	return (String[])accum.toArray(new String[accum.size()]);
    }
    static private void inputError(LineNumberReader in, String line, File file, String msg)
    {
