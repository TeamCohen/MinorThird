...
public class ChiSquareInstanceTransform extends AbstractInstanceTransform
{
  ...
  private ArrayList featValues;//keeps all features
  private Set availableFeatures = new HashSet(); //keeps high score features
...
//        availableFeatures = new HashSet();
  ...
  public String toString(int top){
      ...
      for (int j=0; j<maxIndex; j++)
      {
          Feature f = ((Pair)featValues.get(j)).feature;
          double val = ((Pair)featValues.get(j)).value;
  ...
  public Feature[] getTopFeatures(int top){
      ...
      for (int j=0; j<maxIndex; j++)
      {
          ff[j] = ((Pair)featValues.get(j)).feature;
      }
      return ff;
  }
