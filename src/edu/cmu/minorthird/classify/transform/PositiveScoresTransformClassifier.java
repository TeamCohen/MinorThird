...
public class PositiveScoresTransformClassifier implements Classifier,Serializable{
	...
	public ClassLabel classification(Instance instance)
    {
      ...
      return s>=threshold ? ClassLabel.positiveLabel(th) : ClassLabel.negativeLabel(th);
      //need to change
    ...
    private double calculateMinFeatScore(Dataset data, double percent){
    	...
    	for (Feature.Looper i=hyp.featureIterator(); i.hasNext(); ){
    		Feature f = i.nextFeature();
    		double aa = hyp.featureScore(f);
    		if(lastMax < aa) lastMax = aa;
    	}    	
  ...
  private static void usage() { System.out.println("PositiveScoresTransformClassifier dataset"); }
	...
	public static void main(String[] args) {
	    ...
	    SpanFeatureExtractor fe = edu.cmu.minorthird.text.learn.SampleFE.BAG_OF_LC_WORDS;
	    TextLabels labels;
		try {
	       labels = LineProcessingUtil.readBsh(new File("C:/m3test/total/data/"), new File("C:/m3test/total/env/all"+mytag+".env"));
	       ...
	       for (Span.Looper it = tb.documentSpanIterator(); it.hasNext();){
	        ...
	        mylabel = labels.hasType(docspan, mytag) ? ClassLabel.binaryLabel(+1)  : ClassLabel.binaryLabel(-1);
...
//       ClassifierLearner learner = new NaiveBayes();
		 ...
		 for (int k=0; k<s.getNumPartitions(); k++) {
	    	...
	    	System.out.println("splitting with "+split+", preparing to train on "+trainData.size()
								 +" and test on "+testData.size());
			Classifier cc = new DatasetClassifierTeacher(trainData).train(learner);
		    
		    //	apply transformation
			Classifier cc_transformed = new PositiveScoresTransformClassifier(cc, trainData);
