...
public class TweakedLearner extends BatchBinaryClassifierLearner {
	...
	public Classifier batchTrain(Dataset dataset) {
		...
		BinaryClassifier bc = (BinaryClassifier) new 
		DatasetClassifierTeacher(m_dataset).train(innerLearner);
	...
	{
		int counter = 0;
		for(Example.Looper i=m_dataset.iterator(); i.hasNext(); counter++) 
		{
			Example ex = i.nextExample();
			ClassLabel predicted = innerLearner.getBinaryClassifier().classification(ex);
			...
			tweakingTable.add( new Row(ex.asInstance(), ex.getLabel(), 
										predicted, ClassLabel.negativeLabel(-1.0) ) );
	...
	{
		if (!isBinary) return ILLEGAL_VALUE; // to be on the safe side
		...
		Collections.sort(
				...
				new Comparator() {
					public int compare(Object a, Object b) {
						return MathUtil.sign( (	(Row)b).orig_predicted.posWeight() - 
											   ((Row)a).orig_predicted.posWeight() );
					}
				});
	}
	...
	private static class Row implements Serializable
	{
		...
		{
			instance=i; actual=a; orig_predicted=orig_p; tweak_predicted = tweak_p;  
		}
	 ...
	 ******************************************************************** 
	 */
	public static class TweakedClassifier extends BinaryClassifier 
	implements Serializable, Visible {
		...
		public Viewer toGUI() {
			final Viewer v = new ComponentViewer() {
				public JComponent componentFor(Object o) {
					...
					mainPanel.add(
							new JLabel("Optimal threshold for TweakedClassifier="+c.m_threshold),
							BorderLayout.NORTH);
					mainPanel.add(
							new JLabel("Original classifier before tweaking:"),
							BorderLayout.CENTER);
	...
	public static void main(String[] args) {
		...
		TweakedLearner tl = new TweakedLearner(nb,3.0);
