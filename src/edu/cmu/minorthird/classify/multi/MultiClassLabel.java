...
public class MultiClassLabel implements Serializable
{
	...
	for(int i=0; i<dimensions; i++) {
	    positive[i] =  ExampleSchema.POS_CLASS_NAME.equals(labels[i].bestClassName()); 
	}
	return positive;
    }
	...
	for(int i=0; i<dimensions; i++) {
	    negative[i] = ExampleSchema.NEG_CLASS_NAME.equals(labels[i].bestClassName()); 
	}
	return negative;
    }
	...
	if (dimensions != otherLabel.numDimensions()) throw new IllegalArgumentException("Number of Dimensions do not match");
