...
public class RandomTreeLearner extends BatchBinaryClassifierLearner
{
  ...
  public static class BestOfNRandomTreeSplitter implements TreeSplitter {
    ...
    public Object[] getSplit(List<Example> dataset,int depth, Vector<Feature> unusedFeatures) {
      ...
      for(int i = 0; i < featureCount && i < unusedFeatures.size(); i++) {
        ...
        for (Example example: dataset) {
          if (example.getWeight(f)>=threshold)  trueData.add(example);
          else                                  falseData.add(example);
        }

        double i_gain = entropy(trueData.size(), falseData.size(), trueData.size(),falseData.size());
  ...
  public DecisionTree batchTrain(List<Example> dataset,int depth, Vector<Feature> unusedFeatures) {
    ...
    if(result.length == 4) { // The splitter did the splitting, just use the result
      ...
      for (Example example: dataset) {
        if(example.getWeight(bestFeature)>=bestThreshold) trueData.add(example);
        else                                              falseData.add(example);
      }
    }
