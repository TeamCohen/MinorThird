...
{	
     ...
     * 
     */
    public VisibleSVM(svm_model          model, 
	...
	for (Feature.Looper flidx = m_examples[exampleIndex].featureIterator(); flidx.hasNext(); ) {
     ...
     * 
     */
    private void initialize(svm_model          model, 
			    ExampleSchema      schema) 
    {
	...
	m_posIndicator = 
	    ((labelTypes.length == 2 && m_classStrLabels[0].equals(ExampleSchema.POS_CLASS_NAME)) || 
	     labelTypes.length > 2) ? 1 : -1;
	...
	if( m_hyperplanes.length == 1 ) {
		...
		m_exampleWeightLabels[index][0] = df.format(m_examples[index].getWeight());
	    }
	} else {
	    // For multiClass
	    for (int index = 0; index < m_exampleWeightLabels.length ; ++index) {
     ...
     * 
     */
    private void setExamples(svm_model          model, 
			     FeatureIdFactory   idFactory)
    {
	...
	for (int index = 0; index < numOfSupportVectors; ++index) {
	    ...
	    // construct example
	    Instance iTemp = SVMUtils.nodeArrayToInstance(
							  supportVectorsTemp[index], idFactory);
	    m_examples[index] = new Example(iTemp, clTemp, weightTemp);
			
	    // check if next SV belongs to different class label
	    if( index == (upperBoundIndex - 1) && 
		index != (numOfSupportVectors - 1)){
	...
	for (int index = 0; index < numOfSupportVectors; ++index) {
	    ...
	    for (Feature.Looper flidx = m_examples[index].featureIterator(); flidx.hasNext();) {
	...
	for(int iIdx = 0 ; iIdx < labelTypes.length ; ++iIdx) {
		...
		for(int exampleIdx = label0startedSVindex ; 
		    exampleIdx < (label0startedSVindex + label0svsCount) ;
		    ++exampleIdx) {

		    m_examples[exampleIdx].setWeight(coef[jIdx - 1][exampleIdx]);

		    m_exampleWeightLabels[exampleIdx][hpIndex] = df.format(coef[jIdx - 1][exampleIdx]);
					
		    addInfoToHyperplane(exampleIdx, hpIndex);
		}
			    
		for(int exampleIdx = label1startedSVindex ; 
		    exampleIdx < (label1startedSVindex + label1svsCount) ;
		    ++exampleIdx) {

		    m_examples[exampleIdx].setWeight(coef[iIdx][exampleIdx]);

		    m_exampleWeightLabels[exampleIdx][hpIndex] = df.format(coef[iIdx][exampleIdx]);
	...
	Viewer svsGui = new ControlledViewer(new SVsViewer(), new SVsControls(m_hpLabels.length, m_hpLabels));
	...
	public SVsControls(int numButtons, String[] buttonLabels) {
		...
		weightTypeButton[0] = addButton("weight(" + m_buttonLabels[0] + ")", group, true);
		...
		weightTypeButton[index] = addButton("weight(" + m_buttonLabels[index] + ")", group, false);
     ...
     * 
     */
    static private class SVsViewer 
     ...
     *
     */
    static private class SVMHyperPlaneViewer 
	extends ComponentViewer {
