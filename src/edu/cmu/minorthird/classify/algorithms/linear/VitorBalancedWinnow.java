...
public class VitorBalancedWinnow extends OnlineBinaryClassifierLearner implements Serializable {
...
//		this(1.5, 0.5, true);//voted: recommended
	...
	public void addExample(Example example2) {			
		...
		double y_t_hat = pos_t.score(example.asInstance()) - neg_t.score(example.asInstance()) - theta;
		...
		if(y_t * y_t_hat<=margin){//if error occurred
			...
			if(example.getLabel().isPositive()){
				for (Feature.Looper j=example.featureIterator(); j.hasNext(); ) {
				    ...
				    if(pos_t.featureScore(f)<W_MAX) pos_t.multiply(f,(1+example.getWeight(f))*(alpha));
				    if(neg_t.featureScore(f)>W_MIN) neg_t.multiply(f,(1-example.getWeight(f))*(beta));
				}
			}
			else{
				for (Feature.Looper j=example.featureIterator(); j.hasNext(); ) {
				    Feature f = j.nextFeature();
				    if(pos_t.featureScore(f)>W_MIN) pos_t.multiply(f,(1-example.getWeight(f))*(beta));
				    if(neg_t.featureScore(f)<W_MAX) neg_t.multiply(f,(1+example.getWeight(f))*alpha);
				}				
			}		
		}
