...
public class KWayMixtureLearner extends BatchClassifierLearner
{
   ...
   public Classifier batchTrain(Dataset dataset)
   {
      ...
      for(Example.Looper i=dataset.iterator(); i.hasNext(); )
      {
         ...
         if (idx != classIndex) { System.out.println("Buzz! Error: incompatible class indeces ..."); System.exit(1); }
      ...
      for( Feature.Looper floo=index.featureIterator(); floo.hasNext(); )
      {
         ...
         if (MODEL.equals("Naive-Bayes"))
         {
            ...
            for (int j=0; j<numberOfClasses; j++)
            {
               ...
               {
                  Estimate mean = Estimators.estimateNaiveBayesMean( 1.0,numberOfFeatures,sum( (double[])featureMatrix.get(j) ),examplesGivenClass[j] );
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,mean );
               }
               else if (PARAMETERIZATION.equals("weighted-mean"))
               {
                  ...
                  Estimate mean = Estimators.estimateNaiveBayesWeightedMean( countsFeatureGivenClass,countsGivenExample,1.0/numberOfFeatures,SCALE );
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,mean );
               }
            }
         ...
         else if (MODEL.equals("Binomial"))
         {
            ...
            for (int j=0; j<numberOfClasses; j++)
            {
               ...
               else if (PARAMETERIZATION.equals("mu/delta"))
               {
                  double[] countsFeatureGivenClass = (double[])featureMatrix.get(j);
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
                  Estimate mudelta = Estimators.estimateBinomialMuDelta( countsFeatureGivenClass,countsGivenExample,1.0/numberOfFeatures,SCALE );
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,mudelta );
               }
            }
         ...
         else if (MODEL.equals("Poisson"))
         {
            ...
            for (int j=0; j<numberOfClasses; j++)
            {
               ...
               probabilityOfOccurrence = estimateClassProbMLE( 1.0,(double)numberOfClasses,examplesGivenClass[j],numberOfExamples );
               ((MultinomialClassifier)mc).setClassParameter( j,probabilityOfOccurrence );

               if (PARAMETERIZATION.equals("default")|PARAMETERIZATION.equals("weighted-lambda"))
               {
                  double[] countsFeatureGivenClass = (double[])featureMatrix.get(j);
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
                  Estimate lambda = Estimators.estimatePoissonWeightedLambda( countsFeatureGivenClass,countsGivenExample,1.0/numberOfFeatures,SCALE );
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,lambda );
               }
               else if (PARAMETERIZATION.equals("lambda"))
               {
                  Estimate lambda = Estimators.estimatePoissonLambda( 1.0/SCALE,numberOfFeatures,sum( (double[])featureMatrix.get(j) ),countsGivenClass[j]/SCALE ); // SCALE is HERE !!!
         ...
         else if (MODEL.equals("Negative-Binomial"))
         {
            ...
            for (int j=0; j<numberOfClasses; j++)
            {
               ...
               if (PARAMETERIZATION.equals("default")|PARAMETERIZATION.equals("mu/delta"))
               {
                  double[] countsFeatureGivenClass = (double[])featureMatrix.get(j);
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
         ...
         else if (MODEL.equals("Mixture"))
         {
            ...
            for (int j=0; j<numberOfClasses; j++)
            {
               ...
               if (mean>var) { model="Binomial"; } // Poisson is used in case d=0
               else if (mean<=var) { model="Negative-Binomial"; } // Poisson is used in case d=0
               ...
               {
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
                  Estimate m = Estimators.estimateNaiveBayesWeightedMean( countsFeatureGivenClass,countsGivenExample,1.0/numberOfFeatures,SCALE );
         ...
         else if (MODEL.equals("Dirichlet-Poisson MCMC"))
         {
            ...
            if (PARAMETERIZATION.equals("default")|PARAMETERIZATION.equals("weighted-lambda"))
            {
               for (int j=0; j<numberOfClasses; j++)
               {
                  ...
                  double[] countsFeatureGivenClass = (double[])featureMatrix.get(j);
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
                  lambda[j] = Estimators.estimatePoissonWeightedLambda( countsFeatureGivenClass,countsGivenExample,1.0/numberOfFeatures,SCALE );
               ...
               double sigSD = ((Double) lambda[0].getPms().get("lambda") ).doubleValue() +((Double) lambda[1].getPms().get("lambda") ).doubleValue();
               Estimate[] postLambdas = Estimators.mcmcEstimateDirichletPoissonTauSigma
                    (lambda, new double[]{1e-7,1e-7}, new double[]{1.0,150}, sumCountsFeatureGivenClass[0], sumCountsFeatureGivenClass[1], sumCountsGivenExample[0], sumCountsGivenExample[1], new double[]{2.0,1.0}, 0.075, sigSD/10.0, 100);

               for (int j=0; j<numberOfClasses; j++)
               {
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,postLambdas[j] );
               }
            }
            else if (PARAMETERIZATION.equals("lambda"))
            {
               for (int j=0; j<numberOfClasses; j++)
               {
                  ...
                  double[] countsFeatureGivenClass = (double[])featureMatrix.get(j);
                  double[] countsGivenExample = (double[])exampleWeightMatrix.get(j);
                  lambda[j] = Estimators.estimatePoissonLambda( 1.0,numberOfFeatures,sum( (double[])featureMatrix.get(j) ),countsGivenClass[j] );
                  //((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,j,lambda );
               ...
               Estimate[] postLambdas = Estimators.mcmcEstimateDirichletPoissonTauSigma
                   (lambda, new double[]{1e-7,1e-7}, new double[]{1e-7,150}, sumCountsFeatureGivenClass[0], sumCountsFeatureGivenClass[1], sumCountsGivenExample[0], sumCountsGivenExample[1], new double[]{2.0,1.0}, 0.1, 0.5, 100);

               for (int j=0; j<numberOfClasses; j++)
               {
                  ((MultinomialClassifier)mc).setFeatureGivenClassParameter( ft,0,postLambdas[j] );
               }
            }
            ((MultinomialClassifier)mc).setFeatureModel( ft,"Dirichlet-Poisson MCMC" );
         }
      }
