...
//classification(RealRelationalDataset dataset)
	...
	public double[] stDevErrorsByClass()
	{
		...
		for (int k=0; k<cvFolds; k++)
		{
			...
			for (int i=0; i<K; i++)
			{
				stdev[i] += Math.pow( errorsByClass[i]/numerOfExamplesByClass[i]-mean[i],2 ) / ((double)cvFolds);
			}
		}
		for (int i=0; i<K; i++)
		{
	...
	{
		if (!isBinary) return -1;
		if (numberOfPositiveExamples()==0) return 1.0; // special case
	...
	{
		if (!isBinary) return -1;
		if (numberOfInstances()==0) return Double.NaN; // undefined!
	...
	{
		if (!isBinary) return -1;
		if (numberOfPositiveExamples()==0) return 1.0;
		double maxF1 = 0;
		Matrix m = precisionRecallScore();
		for (int i=0; i<m.values.length; i++) {
	...
	{
		if (cachedTPFPMatrix!=null) return cachedTPFPMatrix;
	    ...
	    if (getEntry(i).actual.isPositive()) {
				...
				allActualNeg++;
				if (notFoundYet) { firstIndexOfActualNeg = i; notFoundYet = false; }
	    }
	    pc.progress();
		}
		pc.finished();
		...
		ProgressCounter pc2 = new ProgressCounter("computing statistics","examples",entryList.size());
	    ...
	    if (allActualPos>0) tpf = truePosSoFar/allActualPos;
	    if (allActualNeg>0) fpf = falsePosSoFar/allActualNeg;
	    if (i==0)
			{
		    result[0][0] = 0.0;
		    result[0][1] = 0.0;
	...
	public Viewer toGUI()
	{
		...
		if (isBinary) main.addSubView(" ROC & AUC ",new ROCViewer());
