...
//		Map queryMap = listsWithOneExampleEach( splitIntoRankings(traindata) );
		Map queryMap = splitIntoRankings(traindata);
		ProgressCounter pc = new ProgressCounter("ListNet training", "epoch", numEpochs);
...
//				System.out.println("SELF-entropy = "+selfEntropy);
			...
			if(x < minCEImprovement) {
				outcount++;
				if(outcount > maxNumBadSteps) return (Hyperplane)ar.get(ar.size()-1);
...
//		System.out.println(w);
	 ...
	 * @return
	 */
	private Hyperplane calculateGradient(List list){
		Hyperplane hyp = new Hyperplane();		
		//calculates first term
		for(int i=0; i<list.size(); i++){
			Instance ins = (Instance)list.get(i);
			//for each feature in this example
			for(Feature.Looper loop = ins.featureIterator(); loop.hasNext();){
				Feature f = loop.nextFeature();
	 ...
	 * @param w
	 */
	private void initialize(List list,Hyperplane w){
		...
		for(int i=0; i<list.size(); i++){
			...
			if(ex.getLabel().isPositive()) sumY+= Math.exp(RELEVANT);
			else sumY+= Math.exp(NON_RELEVANT);			
			sumZ += Math.exp(w.score(ex));			
		}		
		for(int i=0; i<list.size(); i++){
			Example ex = (Example)list.get(i);
			double tmp = (ex.getLabel().isPositive())? Math.exp(RELEVANT):Math.exp(NON_RELEVANT);;
	...
	private Dataset separateDevData(Dataset data){
		Dataset.Split split = data.split(new CrossValSplitter(5));
	 ...
	 */
	double calculateLoss( Map queryMap, Hyperplane w){
		double ce = 0;
//		selfEntropy = 0.0;
	    for (Iterator i=queryMap.keySet().iterator(); i.hasNext(); ) {
	    	String subpop = (String)i.next();
